...
it. Clicking a button again takes it back off. **Clear**, next to **Copy table**, empties the bar and
shows everything.
...
By name - type any part of the name after the filter, capitals don't matter:
- `--customer "ace hdw"` - customer. Put quotes around a name with spaces.
- `--shipto morrow` - ship to.
...
Click **Copy table**, then paste into your email with **Ctrl+V**. The whole table comes along,
